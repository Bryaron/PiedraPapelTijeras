...
function infoCharacters(userCharacter, pcCharacter){
    ...
    const infoUserCharacter = {
        ...
        imagen : window.getComputedStyle(userCharacter).getPropertyValue('background-image'),
        id: userCharacter.getAttribute("id")
    } 

    const infoPcCharacter = {
        nombre : pcCharacter.classList[1],
        imagen : window.getComputedStyle(pcCharacter).getPropertyValue('background-image'),
...
function fight(user, pc) {
    ...
    if(user.id == pc.id){
        ...
        fighting.querySelector('.user').style.backgroundImage = user.imagen;
        fighting.querySelector('.pc').style.backgroundImage = pc.imagen;
